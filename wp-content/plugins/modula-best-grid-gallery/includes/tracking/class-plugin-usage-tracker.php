	...
	class Modula_Plugin_Usage_Tracker {
		...
		public function __construct(
			...
			if( basename( $this->plugin_file, '.php' ) != 'functions' ) {
				$this->plugin_name = 'modula-best-grid-gallery';
			} else {
		...
		public function get_galleries() {
			...
			$args = array( 'post_type' => 'modula-gallery', 'numberposts' => '-1');
			$galleries = get_posts( $args );
			$formatted_galleries = array() ;

			$license_key = get_option( 'modula_pro_license_key', false);

			$site_url = base64_encode( site_url() );
			
			foreach ( $galleries as $gallery ) {

				if ( isset ( $gallery->post_title ) && strlen( $gallery->post_title) != 0 ) {
					$formatted_galleries[ $gallery->ID ]['title']     = $gallery->post_title;
				}
				
				$formatted_galleries[ $gallery->ID ]['license']       = $license_key;
				$formatted_galleries[ $gallery->ID ]['site_url']      = $site_url;
				$formatted_galleries[ $gallery->ID ]['id']            = $gallery->ID;
				$formatted_galleries[ $gallery->ID ]['last_modified'] = $gallery->post_modified;
				$formatted_galleries[ $gallery->ID ]['settings']      = get_post_meta( $gallery->ID, 'modula-settings', true);
				$formatted_galleries[ $gallery->ID ]['last_modified'] = $gallery->post_modified;
